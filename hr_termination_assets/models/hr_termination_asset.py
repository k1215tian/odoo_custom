...
class HrTerminationAsset(models.Model):
    ...
    def _onchange_return_logic(self):
        ...
        for rec in self:
            ...
            if rec.qty > 0 and rec.qty_return < rec.qty:
                ...
                rec.compensation_amount = missing_qty * \
                    (rec.asset_amount / rec.qty)
        ...
        for rec in self:
            if rec.qty < 1:
                raise ValidationError(
                    "Quantity (qty) tidak boleh kurang dari 1.")
            if rec.qty_return < 0:
                raise ValidationError(
                    "Quantity returned (qty_return) tidak boleh negatif.")
            if rec.qty_return > rec.qty:
                raise ValidationError(
                    "Quantity returned (qty_return) tidak boleh lebih besar dari Quantity (qty).")
    ...
    _sql_constraints = [
        ('asset_name_required', 'CHECK(asset_name IS NOT NULL)',
